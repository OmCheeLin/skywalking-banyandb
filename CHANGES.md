...
- Use `topic` instead of `session_id` as the Prometheus label on liaison `queue_sub` chunk-ordering counters to avoid unbounded metric cardinality; document the metrics in `docs/operation/observability.md` and add a **Liaison queue_sub chunked sync** row to `docs/operation/grafana-cluster.json`.
